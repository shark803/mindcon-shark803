"""Generate MindRecord for BERT finetuning runner: Mindcon text clasification."""
import os
import json
import csv
...
def parse_args():
    parser = ArgumentParser(description="Generate MindRecord for bert task: MindCon")
    parser.add_argument("--data_dir", type=str, default="/home/ubuntu/sda.soft/msdev_projs/mind_con/mindcon_text_classification",
                        help="The input data dir. Should contain the .txt files (or other data files) for the task.")
    parser.add_argument("--task_name", type=str, default="mindcon", help="The name of the task to train.")
    parser.add_argument("--vocab_file", type=str, default="/home/ubuntu/sda.soft/msdev_projs/mind_con/vocab.txt",
                        help="The vocabulary file that the BERT model was trained on.")
    parser.add_argument("--output_dir", type=str, default="/home/ubuntu/sda.soft/msdev_projs/mind_con/output",
...
def file_based_convert_examples_to_features(
    ...
    # }
    # writer = FileWriter(output_file, overwrite=True,shard_num=1)
...
class MindconProcessor(DataProcessor):
    """Processor for the MRPC data set (GLUE version)."""

    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_txt(os.path.join(data_dir, "train/train.txt")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_txt(os.path.join(data_dir, "train/dev.txt")), "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_txt(os.path.join(data_dir, "test/test.txt")), "test")
...
def main():
    ...
    if args.do_train:
        ...
        train_file = os.path.join(args.output_dir, "train.tfrecord")
    ...
    if args.do_eval:
        ...
        eval_file = os.path.join(args.output_dir, "dev.tfrecord")
        file_based_convert_examples_to_features(
    ...
    if args.do_predict:
        ...
        predict_file = os.path.join(args.output_dir, "predict.tfrecord")
