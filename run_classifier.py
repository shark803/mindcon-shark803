...
def do_predict(dataset=None, network=None, num_class=2, load_checkpoint_path="",save_predict_results=""):
    """ do eval """
    if load_checkpoint_path == "":
        raise ValueError("Finetune model missed, evaluation task must load finetune model!")
    net_for_pretraining = network(bert_net_cfg, False, num_class)
    ...
    columns_list = ["input_ids", "input_mask", "segment_ids", "label_ids"]
    with open(os.path.join(save_predict_results,"comment_result.txt"),'w') as f:
        for data in dataset.create_dict_iterator(num_epochs=1):
            input_data = []
            for i in columns_list:
                input_data.append(data[i])
            ...
            f.write(str(np.argmax(logits[0])) + "\n")
...
def run_classifier():
    ...
    if args_opt.do_predict.lower() == "true" and args_opt.eval_data_file_path == "":
        raise ValueError("'test_data_file_path' must be set when do test task")
    ...
    if args_opt.do_predict.lower() == "true":
        ds = create_classification_dataset(batch_size=args_opt.eval_batch_size,
                                           assessment_method=assessment_method,
                                           data_file_path=args_opt.test_data_file_path,
                                           schema_file_path=args_opt.schema_file_path,
                                           do_shuffle=(args_opt.eval_data_shuffle.lower() == "false"))
        do_predict(ds, BertCLS, args_opt.num_class, load_finetune_checkpoint_path, args_opt.save_predict_results)
